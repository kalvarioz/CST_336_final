...
import { GetObjectCommand, ListObjectsV2Command, DeleteObjectCommand, HeadObjectCommand } from "@aws-sdk/client-s3";

// --- Config imports (infrastructure) ---
import db                        from "./config/db.mjs";
import { r2Client, BUCKET_NAME } from "./config/r2.mjs";
import { requireLogin, requireAdmin } from "./config/auth.mjs";
import { searchRelease, lookupRelease, enrichSong, cleanArtist } from "./config/musicbrainz.mjs";
...
app.use((req, res, next) => {
    res.locals.user = req.session.user_id
        ? {
              ...
              isAdmin:     req.session.is_admin || false,
          }
        : null;
    next();
});
...
app.post("/api/auth/signup", async (req, res) => {
    try {
        ...
        // 4. Auto-login: set session (new users are never admins)
...
// GET /api/genres — dynamic genre list from the database
// Returns every distinct genre + count, sorted by count desc.
// The frontend uses this to populate dropdown menus dynamically.
...
app.post("/api/songs/enrich-all", requireLogin, async (req, res) => {
    try {
        ...
        // Group by PRIMARY artist + album to minimize API calls.
        // cleanArtist strips featured artists so all songs on the
        // same album get grouped together regardless of features.
        const albumMap = new Map();
        for (const song of songs) {
            const primary = cleanArtist(song.artist);
            const key = `${primary}|||${song.album}`;
            if (!albumMap.has(key)) {
                albumMap.set(key, { artist: primary, album: song.album, songIds: [] });
            }
...
//
// All routes here require both login AND admin role.
// These let admins manage users, songs, and view R2 storage.
//
// Rubric: UPDATE SQL with at least 3 fields (user editing),
//         INSERT (add songs), and database management.
// ============================================================
    ...
    try {
        const [rows] = await db.query(
            `SELECT user_id, username, email, display_name, favorite_genre,
                    is_admin, show_recent, created_at
...
// PUT /api/admin/users/:id — update a user (3+ fields)
app.put("/api/admin/users/:id", requireLogin, requireAdmin, async (req, res) => {
    const userId = parseInt(req.params.id, 10);
    if (isNaN(userId)) return res.status(400).json({ error: "Invalid user ID" });

    try {
        const { display_name, email, favorite_genre, is_admin } = req.body;

        // Prevent removing your own admin access
        if (userId === req.session.user_id && !is_admin) {
            return res.status(400).json({ error: "Cannot remove your own admin access" });
        }

        // UPDATE with 4 fields (exceeds rubric requirement of 3)
        await db.query(
            `UPDATE users
             SET display_name = ?, email = ?, favorite_genre = ?, is_admin = ?
             WHERE user_id = ?`,
            [display_name, email, favorite_genre || "All", is_admin ? 1 : 0, userId]
        );

        res.json({ message: "User updated" });
    } catch (err) {
        console.error("[ADMIN PUT /users]", err);
...
// GET /api/admin/songs — list all songs (with cloud_key visible to admin)
app.get("/api/admin/songs", requireLogin, requireAdmin, async (req, res) => {
    try {
        const [rows] = await db.query(
        ...
        console.error("[ADMIN GET /songs]", err);
        res.status(500).json({ error: "Failed to load songs" });
    }
});

// PUT /api/admin/songs/:id — update song metadata (3+ fields)
app.put("/api/admin/songs/:id", requireLogin, requireAdmin, async (req, res) => {
    const songId = parseInt(req.params.id, 10);
    if (isNaN(songId)) return res.status(400).json({ error: "Invalid song ID" });
    ...
    try {
        ...
        // UPDATE with 4 fields
        await db.query(
            `UPDATE songs
             SET title = ?, artist = ?, album = ?, genre = ?
             WHERE song_id = ?`,
            [title, artist, album || null, genre || "Other", songId]
        );

        res.json({ message: "Song updated" });
    } catch (err) {
        console.error("[ADMIN PUT /songs]", err);
        res.status(500).json({ error: "Failed to update song" });
    }
});

// POST /api/admin/songs — add a song record (INSERT)
app.post("/api/admin/songs", requireLogin, requireAdmin, async (req, res) => {
    try {
        const { title, artist, album, genre, duration_sec, cloud_key } = req.body;

        if (!title || !artist || !cloud_key) {
            return res.status(400).json({ error: "Title, artist, and cloud_key are required" });
        }

        const [result] = await db.query(
            `INSERT INTO songs (title, artist, album, genre, duration_sec, cloud_key)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [title, artist, album || null, genre || "Other", duration_sec || 0, cloud_key]
        );

        res.status(201).json({ message: "Song added", song_id: result.insertId });
    } catch (err) {
        console.error("[ADMIN POST /songs]", err);
        res.status(500).json({ error: "Failed to add song" });
    }
});

// DELETE /api/admin/songs/:id — delete a song record
app.delete("/api/admin/songs/:id", requireLogin, requireAdmin, async (req, res) => {
    const songId = parseInt(req.params.id, 10);
    if (isNaN(songId)) return res.status(400).json({ error: "Invalid song ID" });
    ...
    try {
        ...
        console.error("[ADMIN DELETE /songs]", err);
...
// GET /api/admin/r2/files — list files in the R2 bucket
app.get("/api/admin/r2/files", requireLogin, requireAdmin, async (req, res) => {
    try {
        const prefix = req.query.prefix || "music/";
        ...
        const r2Response = await r2Client.send(cmd);
        const files = (r2Response.Contents || []).map(obj => ({
            key:          obj.Key,
            size:         obj.Size,
        ...
        console.error("[ADMIN R2 /files]", err);
        res.status(500).json({ error: "Failed to list R2 files" });
    }
});

// DELETE /api/admin/r2/files — delete a file from R2
app.delete("/api/admin/r2/files", requireLogin, requireAdmin, async (req, res) => {
    const { key } = req.body;
...
app.get("/api/admin/stats", requireLogin, requireAdmin, async (req, res) => {
    try {
        ...
        const [[{ songCount }]]    = await db.query("SELECT COUNT(*) AS songCount FROM songs");
        const [[{ enrichedCount }]] = await db.query("SELECT COUNT(*) AS enrichedCount FROM songs WHERE mb_release_id IS NOT NULL");
        const [[{ genreBreakdown }]] = await db.query(
            `SELECT JSON_ARRAYAGG(JSON_OBJECT('genre', genre, 'count', cnt))
             AS genreBreakdown
             FROM (SELECT genre, COUNT(*) AS cnt FROM songs GROUP BY genre ORDER BY cnt DESC) sub`
        );
        ...
        res.json({
            ...
            genres:   JSON.parse(genreBreakdown || "[]"),
        });
    } catch (err) {
