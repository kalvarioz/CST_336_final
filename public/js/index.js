...
// // Uses the local /api/musicbrainz/search endpoint which
// // proxies the request to the external MusicBrainz Web API.
// // This counts as our external API usage for the rubric.
// async function fetchMusicBrainzInfo(song) {
//     // If we already have an mb_release_id, use the direct lookup
...
//             // Direct lookup by known ID
//             const resp = await fetch(`/api/musicbrainz/release/${song.mb_release_id}`);
//             if (resp.ok) {
//                 const data = await resp.json();
//                 if (!data.message) releaseData = data;  // "message" means not found
//             }
//         } else {
...
//                 album:  song.album,
//             });
//             const resp = await fetch(`/api/musicbrainz/search?${params.toString()}`);
//             if (resp.ok) {
//                 const searchResult = await resp.json();
//                 if (searchResult && searchResult.mb_release_id) {
//                     // We found a match — now do a full lookup to get the tracklist
//                     const lookupResp = await fetch(`/api/musicbrainz/release/${searchResult.mb_release_id}`);
//                     if (lookupResp.ok) {
...
//             }
//             // If we got a cover art URL from search and current song doesn't have one,
//             // try setting it now
//             if (releaseData.mb_release_id && !song.cover_art_url) {
...
function selectSong(songId) {
    ...
    // --- Start streaming audio through the secure proxy ---
    audioPlayer.src = `/api/songs/stream/${songId}`;
    audioPlayer.load();
    audioPlayer.play().catch(err => {
        // Autoplay might be blocked by browser — that's OK,
...
// ============================================================
// INITIAL LOAD — fetch all songs when the page loads
// ============================================================
loadSongs();
